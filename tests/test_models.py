...
def make_line(amount, acct_full='Acct', desc='Desc'):
    return Line(date='2026-01-02', txn_id='h', desc=desc, memo='',
                acct_full=acct_full, acct_short='Acct', amount=amount)
...
class TestEntry(unittest.TestCase):
    ...
    def test_split_tracks_size(self):
        e = Entry()
        e.add_line(make_line('-100.00'))
        e.add_line(make_line('100.00'))
        self.assertEqual(e.size, 2)
        self.assertFalse(e.split)

        e.add_line(make_line('0.00'))
    ...
    def test_constructed_from_lines(self):
        e = Entry([make_line('-100.00'), make_line('100.00')])
        self.assertEqual(e.size, 2)
        self.assertEqual(e[0].amount, '-100.00')
